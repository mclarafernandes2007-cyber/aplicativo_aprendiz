    ...
    if not st.session_state.gerenciador_logado:
        ...
        if not gs["jogo_iniciado"]:
            ...
            if doc_atual_mgr:
                ...
                # Nome do documento sempre visível
                st.markdown(
                    ...
                    f'<div style="color:#ffffff;font-size:1.3rem;font-weight:bold">{doc_atual_mgr}</div>'
                    f'</div>',
                    unsafe_allow_html=True,
                )

                # Descrição só aparece após encerrar a rodada
                if gs["rodada_encerrada"] and descricao_mgr:
                    st.markdown(
                ...
                # Quadro de emojis (somente leitura para o gerenciador)
                st.markdown("**🎨 Quadro atual do desenhista:**")
                canvas_html_mgr = build_canvas_html(
                    emojis_atuais=gs["emojis_quadro"],
                    pode_editar=False,
                    paleta=PALETA_EMOJIS,
                )
                emojis_hash_mgr = hash(json.dumps(gs["emojis_quadro"], sort_keys=True))
                canvas_html_mgr = "<!-- mgr:" + str(emojis_hash_mgr) + " -->\n" + canvas_html_mgr
                components.html(canvas_html_mgr, height=320, scrolling=False)
    ...
    if not st.session_state.ja_no_jogo:
        ...
        with col_jogo:
            ...
            if eh_desenhista and not gs["rodada_encerrada"]:
                # Desenhista: canvas editável
                canvas_html = build_canvas_html(
                    emojis_atuais=gs["emojis_quadro"],
                    pode_editar=True,
                    paleta=PALETA_EMOJIS,
                )
                resultado = components.html(canvas_html, height=640, scrolling=False)

                # Captura estado local a cada rerun
                ...
                # Botão de envio real (Streamlit) — salva no estado global e faz rerun
                ...
                # Espectadores e desenhista após encerramento: canvas somente leitura
                # Armazena o hash do quadro que o jogador está vendo atualmente
                emojis_hash_atual = hash(json.dumps(gs["emojis_quadro"], sort_keys=True))
                emojis_hash_visto = st.session_state.get("emojis_hash_visto", None)
                quadro_tem_novidade = (emojis_hash_visto != emojis_hash_atual)

                # Botão de atualizar (para espectadores durante o jogo)
                if not gs["rodada_encerrada"] and not eh_desenhista:
                    if quadro_tem_novidade:
                        if st.button("🔄 Ver quadro atualizado", use_container_width=True, type="primary"):
                            st.session_state["emojis_hash_visto"] = emojis_hash_atual
                            st.rerun()
                    else:
                        st.info("✅ Quadro em dia — aguarde o desenhista enviar novas pistas.")

                # Exibe o canvas com os emojis que o jogador escolheu ver (ou os atuais se encerrado)
                emojis_para_exibir = gs["emojis_quadro"] if (
                    gs["rodada_encerrada"] or eh_desenhista or emojis_hash_visto == emojis_hash_atual
                ) else (
                    # Recarrega o estado do quadro que o jogador tinha confirmado
                    gs["emojis_quadro"] if emojis_hash_visto is None else gs["emojis_quadro"]
                )
                canvas_html = build_canvas_html(
                    emojis_atuais=emojis_para_exibir,
                    pode_editar=False,
                    paleta=PALETA_EMOJIS,
                )
                components.html(canvas_html, height=380, scrolling=False)
