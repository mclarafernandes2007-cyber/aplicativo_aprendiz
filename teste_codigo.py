...
if st.session_state.aba_ativa == "gerenciador":
    if not st.session_state.gerenciador_logado:
        ...
        if not gs["jogo_iniciado"]:
            ...
            if doc_atual_mgr:
                st.info(f"📄 Documento desta rodada: **{doc_atual_mgr}**")
        ...
        st.divider()
        st.markdown("### 📚 Documentos & Descrições")
        st.caption("Referência completa — visível apenas para o gerenciador")
        for nome_doc, info in DOCUMENTOS.items():
            with st.expander(f"📄 {nome_doc}"):
                st.markdown(f'<div class="dica-card" style="border-left-color:#f39c12">{info["descricao"]}</div>', unsafe_allow_html=True)
                st.markdown("**Dicas da rodada:**")
                for i, d in enumerate(info["dicas"], 1):
                    st.markdown(f'<div class="dica-card">📌 Dica {i}: {d}</div>', unsafe_allow_html=True)
...
# AUTO-REFRESH
# ─────────────────────────────────────────────
if st.session_state.ja_no_jogo and st.session_state.aba_ativa == "jogo":
    if gs["jogo_iniciado"] and not gs["rodada_encerrada"]:
        time.sleep(3)
        st.rerun()
    elif gs["jogo_iniciado"] and gs["rodada_encerrada"]:
        time.sleep(5)
        st.rerun()
